...
interface Conjunction {
  ...
  probabilityOfCollision: number;  // Pc, e.g. 2.8e-4
  missDistanceMeters: number;      // 742
  relativeVelocityKms: number;     // 13.7
  relativeSpeedKmh: number;        // 27650
  combinedUncertaintyKm: number;   // 1.28
...
| Feature | Endpoint |
|---|---|
| Critical alert banner (SAT-042 ↔ OBJ-8821: TCA, Pc 2.8e-4, miss 742 m, rel. velocity 13.7 km/s) | `GET /api/v1/conjunctions/critical` → highest-severity active `Conjunction` |
