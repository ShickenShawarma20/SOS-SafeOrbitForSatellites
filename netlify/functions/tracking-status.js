/* SOS — Netlify Function: /.netlify/functions/tracking-status
...
exports.handler = async (event) => {
  ...
  const status = okCount > 0 ? (isStale ? "stale" : "ok") : "error";

  return {
    statusCode: 200,
    headers: { "Content-Type": "application/json", "Cache-Control": "public, max-age=60" },
    body: JSON.stringify({
      status, fetchedAt: new Date().toISOString(), newestEpoch,
      count: FLEET_COUNT, okCount, propagation: "active",
      message: okCount > 0 ? "Orbital data available · propagation active (cached)" : "Orbital data source unavailable",
    }),
  };
};
