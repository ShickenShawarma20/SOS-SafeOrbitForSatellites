...
router.get("/critical", (req, res) => {
    const critical = conjunctions_js_1.conjunctions
        .filter(c => c.severity === "critical" && !c.acknowledged)
        .sort((a, b) => new Date(b.tca).getTime() - new Date(a.tca).getTime())[0];
    if (!critical)
        throw new error_js_1.AppError(404, "NOT_FOUND", "No critical conjunctions");
    res.json(critical);
});
router.get("/summary", (req, res) => {
    const windowHours = parseInt(req.query.window?.replace("h", "")) || 48;
    const cutoff = new Date(Date.now() + windowHours * 3600 * 1000);
    const active = conjunctions_js_1.conjunctions.filter(c => new Date(c.tca) <= cutoff);
    res.json({
...
router.get("/upcoming", (req, res) => {
    ...
    const upcoming = conjunctions_js_1.conjunctions
        .filter(c => !c.acknowledged)
        .sort((a, b) => new Date(a.tca).getTime() - new Date(b.tca).getTime())
        .slice(0, limit);
    res.json({ items: upcoming, total: conjunctions_js_1.conjunctions.length });
});
router.get("/timeline", (req, res) => {
    const windowStr = req.query.window || "±12h";
    const hours = parseInt(windowStr.replace(/[±h]/g, "")) || 12;
    const now = Date.now();
    const events = conjunctions_js_1.conjunctions.map(c => {
...
router.get("/", (req, res) => {
    ...
    const total = conjunctions_js_1.conjunctions.length;
    const items = conjunctions_js_1.conjunctions.slice((page - 1) * limit, page * limit);
    res.json({ items, total, page, limit });
});
router.get("/:id", (req, res) => {
    const c = conjunctions_js_1.conjunctions.find(c => c.id === req.params.id);
    if (!c)
        throw new error_js_1.AppError(404, "NOT_FOUND", `Conjunction ${req.params.id} not found`);
    res.json(c);
});
...
router.get("/:id/history", (req, res) => {
    ...
    if (!c)
        throw new error_js_1.AppError(404, "NOT_FOUND", `Conjunction ${req.params.id} not found`);
    const records = conjunctions_js_1.cdmRecords.filter(r => r.conjunctionId === c.id);
