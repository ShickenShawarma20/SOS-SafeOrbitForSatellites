...
import type { Satellite, PaginatedResponse } from "../types.js";
...
router.get("/", (req, res) => {
  ...
  const response: PaginatedResponse<Satellite> = { items, total, page, limit };
  res.json(response);
});

router.get("/:id", (req, res) => {
  const sat = satellites.find(s => s.id === req.params.id);
  if (!sat) throw new AppError(404, "NOT_FOUND", `Satellite ${req.params.id} not found`);
  res.json(sat);
});
...
router.get("/:id/events", (req, res) => {
  const items = feedEvents.filter(e => e.satelliteId === req.params.id);
  res.json(items);
});
