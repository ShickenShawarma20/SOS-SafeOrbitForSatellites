/* SOS — Vercel serverless function: /api/v1/tracking/fleet
 * Returns cached TLE data for the ISRO satellite fleet.
 * Self-contained (no cross-directory imports) for Vercel compatibility.
 * Uses a module-level cache that persists across warm invocations.
 */
import type { VercelRequest, VercelResponse } from "@vercel/node";

// ---- ISRO fleet config (inlined from server/src/data/isro-fleet.ts) ----
interface FleetMember { noradId: number; name: string; alias?: string; operator: string; category: string; source: string; }
const ISRO_FLEET: FleetMember[] = [
...
// ---- TLE snapshot (inlined from server/src/data/tle-snapshot.json) ----
interface SnapshotTle { noradId: number; name: string; line1: string; line2: string; }
const SNAPSHOT_TLES: SnapshotTle[] = [
  ...
  { noradId: 39086, name: "SARAL", line1: "1 39086U 13009A   26238.84756950  .00000800  00000+0  50000-4 0  9991", line2: "2 39086  98.5000 245.0000 0010000 115.0000 245.0000 14.50000000900000" },
];

// ---- Cache ----
interface CachedTle {
  noradId: number; name: string; alias?: string; operator: string; category: string;
  source: string; line1: string; line2: string; epoch: string; fetchedAt: string; ok: boolean;
}

let cache: { tles: Map<number, CachedTle>; fetchedAt: string | null; status: string } = {
  tles: new Map(), fetchedAt: null, status: "initializing",
};
let fetching = false;
let lastRefresh = 0;
...
async function refreshFleetTles(): Promise<void> {
  if (fetching) return;
  fetching = true;
  let successCount = 0;
  let failCount = 0;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  res.json({
    ...
    newestEpoch: tles.reduce<string | null>((n, t) => t.epoch && (!n || t.epoch > n) ? t.epoch : n, null),
    count: tles.length,
    okCount: tles.filter((t) => t.ok).length,
    satellites: tles,
  });
}
